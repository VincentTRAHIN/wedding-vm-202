...
<div class="flex min-h-screen items-center justify-center bg-background p-4">
	<Card.Root class="w-full max-w-md">
		<Card.Header class="text-center">
			<Card.Title class="font-serif text-3xl text-primary">Inscription</Card.Title>
			<Card.Description>
				{#if !codeValidated}
					Veuillez entrer le code invité pour accéder à l'inscription.
				{:else}
					Créez votre compte pour accéder à l'espace invités.
				{/if}
			</Card.Description>
		</Card.Header>
		<Card.Content>
			...
			{#if !codeValidated && !form?.codeValid}
				<form
					action="?/verify_code"
					method="POST"
					class="space-y-4"
				...
				>
					<div class="space-y-2">
						<Label for="code">Code Invité</Label>
						<Input
							id="code"
							name="code"
							type="password"
			...
			{:else}
				...
				>
					<input type="hidden" name="code" value="VM2026" />

					<div class="space-y-2">
						<Label for="fullName">Nom et Prénom</Label>
						<Input id="fullName" name="fullName" type="text" placeholder="Jean Dupont" required />
					</div>
